...
class Api 
{
    ...
    public function __construct($config) {
        ...
        $this->get_menu_diy=isset($config['get_menu']) ? $config['get_menu'] : FALSE;
    }
    ...
    public function get_access_token() {
        ...
        if (!$this->valid_access_token($token)) {
            ...
            //在生成新的access_token时创建菜单
            if(!$this->get_menu_diy===FALSE){
                $this->create_menu($this->get_menu_diy,$token) ;
            }else{
                @error_log('Not set get_menu_diy method, Menu will not create', 0);
            }
            // 保存新生成的AccessToken
     ...
     * ```
     */    
    public function create_menu ($json,$access_token=FALSE) {
        @error_log('create_menu',0);
        if($access_token===FALSE){
            $access_token=$this->get_access_token();
        }
        $url = self::API_DOMAIN . 'cgi-bin/menu/create?access_token='.$access_token ;
        @error_log('create_menu:URL'.$url,0);
        $res = HttpCurl::post($url, $json, 'json');
        // 异常处理: 获取时网络错误
        if ($res === FALSE) {
            @error_log('create_menu ::Error'.$res->errcode,0);
            return Error::code('ERR_GET');
        }
        // 判断是否调用成功        
        if ($res->errcode == 0) {
            return array(NULL, $res);
            @error_log('create_menu ::Success',0);
        } else {            
            return array($res, NULL);
            @error_log('create_menu ::Fail',0);
        }
    }
