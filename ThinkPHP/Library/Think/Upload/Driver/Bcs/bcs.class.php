...
class BaiduBCS {
	...
	public function upload_directory($bucket, $dir, $opt = array()) {
		...
		$before_upload_log = "Article directory: bucket[$bucket] upload_dir[$dir] file_sum[$objectCount]";
		...
		foreach ( $objects as $object => $file ) {
			...
			try {
				...
				$this->log ( "Article Failed id[$num/$objectCount]object[$object]file[$file].", $opt );
				$num ++;
				continue;
			}
			if ($response->isOK ()) {
				$result ["success"] ++;
				$this->log ( "Article Success id[$num/$objectCount]object[$object]file[$file].", $opt );
			} else {
		...
		//打印日志并返回结果数组
		$result_str = "\r\n\r\nArticle $dir to $bucket finished!\r\n";
		$result_str .= "**********************************************************\r\n";
		$result_str .= "**********************Result Summary**********************\r\n";
		$result_str .= "**********************************************************\r\n";
		$result_str .= "Article directory :  [$dir]\r\n";
