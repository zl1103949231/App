...
<ul class="breadcrumb">
	...
	<li><a href="<?php echo U('listView');?>?forum_id=<?php echo ($forum_id); ?>">文章列表</a></li>
</ul>
<div class="container" >
	<div  class="col-md-11">
			...
			<div class="panel-body ">
				...
				<table  class="table table-hover student-detail table-display" id="article_table" >
					...
					<?php if(is_array($articleList)): foreach($articleList as $k=>$article): ?><tr id="<?php echo ($article["article_id"]); ?>">
							...
							<td><img src="<?php echo ($article["img"]); ?>" style="height:50px "  ></td>
							<td><?php echo ($article["time"]); ?></td>
							<td><?php echo ($article["source"]); ?></td>
							<td><a class="btn btn-info" href= '<?php echo U("edit");?>?article_id=<?php echo ($article["article_id"]); ?>'> 编辑</a>
								<a class="btn btn-warning" href= '<?php echo U("show");?>?article_id=<?php echo ($article["article_id"]); ?>' target="_blank"> 预览</a>
								<a class="btn btn-danger" onclick="confrim_delete()" > 删除</a>
								<?php if($k == 0): ?><a  disabled="true" class="btn btn-default"> 上移</a>
									<?php else: ?>
									<a href= '<?php echo U("movePostion");?>?article_id=<?php echo ($article["article_id"]); ?>&&direction=up'class="btn btn-default"> 上移</a><?php endif; ?>
